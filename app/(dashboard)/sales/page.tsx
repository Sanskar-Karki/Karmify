...
import {
  getProducts, getStocks, getSales, createSale, getWarehouses, getCategories,
} from "@/app/actions";
...
export default function SalesPage() {
  ...
  async function refresh() {
    const [products, categories, whs, stocks, sales] = await Promise.all([
      getProducts(), getCategories(), getWarehouses(), getStocks(), getSales(),
    ]);
    setProducts(products.filter((p: any) => p.status === "active"));
  ...
  function getStock(productId: string) {
    return stocks.filter(s => s.productId === productId && s.warehouseId === warehouseId).reduce((a, s) => a + s.quantity, 0);
  }
  ...
  if (!mounted) return (
    <Skeleton name="sales" loading={true} fixture={<SalesFixture />}>
      <SalesFixture />
    </Skeleton>
  );

  return (
    <Skeleton name="sales" loading={false} fixture={<SalesFixture />}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={warehouseId} onChange={e => setWarehouseId(e.target.value)} className="px-3 py-2 rounded-xl border border-border bg-card text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              return (
                <button key={p.id} onClick={() => addToCart(p)} disabled={exhausted} className={cn("relative flex flex-col items-start gap-2 p-4 bg-card border rounded-2xl text-left transition-all cursor-pointer group", exhausted ? "opacity-50 cursor-not-allowed border-border/40" : "border-border/80 hover:border-primary/40 hover:shadow-md hover:shadow-primary/5 hover:scale-[1.01]")}>
                  ...
                  <div className="flex items-end justify-between w-full mt-auto">
                    <p className="text-sm font-extrabold text-primary">${p.sellingPrice.toFixed(2)}</p>
        ...
        <div className="lg:col-span-5 sticky top-6 space-y-4">
          <div className="bg-card border border-border/80 rounded-2xl shadow-sm overflow-hidden">
              ...
              {cart.length === 0 ? (
                ...
                cart.map(item => (
                  <div key={item.productId} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <span className="text-xs font-bold w-16 text-right">${(item.unitPrice * item.quantity).toFixed(2)}</span>
            ...
            {/* Customer + Billing */}
            <div className="p-4 border-t border-border/60 space-y-3.5">
              ...
              <div className="bg-muted/30 rounded-xl p-3 space-y-1.5 text-xs">
                ...
                <div className="flex justify-between font-extrabold text-base text-foreground pt-1 border-t border-border/60"><span>Total</span><span>${total.toFixed(2)}</span></div>
              </div>
              ...
              {sales.slice(0, 8).map(s => (
                <div key={s.id} className="flex items-center justify-between px-4 py-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs font-extrabold">${s.totalAmount.toFixed(2)}</p>
          ...
          <div className="bg-white dark:bg-zinc-900 border border-border rounded-2xl shadow-2xl w-full max-w-sm max-h-[90vh] overflow-y-auto">
            <div className="p-6 space-y-4">
              ...
              <div className="border-t border-dashed border-border/60 pt-4 space-y-1.5 text-xs">
                {lastSale.discount > 0 && <div className="flex justify-between text-emerald-600"><span>Discount</span><span>−${lastSale.discount.toFixed(2)}</span></div>}
                <div className="flex justify-between font-extrabold text-base text-foreground pt-1 border-t border-border/60"><span>TOTAL</span><span>${lastSale.totalAmount.toFixed(2)}</span></div>
              </div>
